	...
	<div id="wrap" align="center">
		...
		<form name="frm" method="post" action="memberEdit">
			<table>
				<tr>
					<th>아이디</th>
					<td>${dto.userid}- ${dto.provider} <input type="hidden"
						name="userid" value="${dto.userid}"> <input type="hidden"
						name="provider" value="${dto.provider}"></td>
				</tr>
				...
				<td>
					<<c:choose>
						<c:when test="${empty dto.provider}">
							<input type="password" name="pwd_check" size="20"> *</td>
						</c:when>
						<c:otherwise>
			...
			<br> <br> <input type="submit" value="수정"> <input
				...
				value="목록으로" onclick="location.href='main'"> ${message}
		</form>
	</div>
</body>
</html>