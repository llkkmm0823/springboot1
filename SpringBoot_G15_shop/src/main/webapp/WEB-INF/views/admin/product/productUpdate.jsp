...
<table id="list">
  ...
  <tr><th>베스트상품</th><td>
      <c:choose>
        <c:when test='${productVO.bestyn=="Y"}'>
            <input type="checkbox" name="bestyn" value="O" checked="checked">
        </c:when>
        <c:otherwise>
        	<input type="checkbox" name="bestyn" value="O">
        </c:otherwise>
      </c:choose></td>        
    <th>사용유무</th><td>
      <c:choose>
        <c:when test='${productVO.useyn=="Y"}'>
              <input type="checkbox" name="useyn" value="O" checked="checked">
        </c:when>
      <c:otherwise>
      		<input type="checkbox" name="useyn" value="O">
      	</c:otherwise>
    </c:choose></td></tr>
  ...
  <tr><th>상품이미지</th><td colspan="5">
      현재 이미지 : <img src="product_images/${productVO.image}" width="200pt"><br>
